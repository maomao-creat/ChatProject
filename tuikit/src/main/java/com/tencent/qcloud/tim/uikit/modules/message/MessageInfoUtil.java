...
import com.tencent.qcloud.tim.uikit.CustomMessageData;
import com.tencent.qcloud.tim.uikit.TUIKit;
import com.tencent.qcloud.tim.uikit.TeamBean;
...
public class MessageInfoUtil {
    ...
    private static MessageInfo ele2MessageInfo(final MessageInfo msgInfo, TIMMessage timMessage, TIMElem ele, boolean isGroup) {
        ...
        if (type == TIMElemType.Custom) {
            ...
            CustomMessageData customMessageData = JSON.parseObject(data, CustomMessageData.class);
            ...
            if (data.equals(GROUP_CREATE)) {
                ...
                if (customMessageData.msgType.equals("hongbao")) {
                    msgInfo.setMsgType(MessageInfo.MSG_TYPE_CUSTOM);
                    msgInfo.setExtra("[红包]");
                } else if (customMessageData.msgType.equals("zhuanzhang")) {
                    msgInfo.setMsgType(MessageInfo.MSG_TYPE_CUSTOM);
                    msgInfo.setExtra("[转账]");
                }
//                msgInfo.setMsgType(MessageInfo.MSG_TYPE_CUSTOM);
//                msgInfo.setExtra("["  +customElem.getDesc()+"]");
            }
...
//                    }
//                }
//                user = listToString(GetUser(mList));
//                ToastUtil.toastLongMessage(user);
            } else {
    ...
    public static List<String> GetUser(List<String> list) {
        final    List<String> UserProList = new ArrayList<>();
        //获取用户资料
        TIMFriendshipManager.getInstance().getUsersProfile(list, true, new TIMValueCallBack<List<TIMUserProfile>>() {
            ...
            public void onError(int code, String desc) {
                ...
                //错误码 code 列表请参见错误码表
                Log.e("MessageInfoUtil", "getUsersProfile failed: " + code + " desc");
            }

            @Override
            public void onSuccess(List<TIMUserProfile> result) {
                Log.e("MessageInfoUtil", "getUsersProfile succ");
                for (int i = 0; i < result.size(); i++) {
                    UserProList.add(result.get(i).getNickName());
                }
                Log.e("UserList", UserProList.toString());
            }
        });
