...
public class QiTaJiLuActivity extends ToolBarActivity<WalleJiLuPresenter> implements SendJiLuView, Listener.OnItemClickListener, Listener.LoadMoreListener {
    ...
    void chushihualb() {
        ...
        rvu = new RecyclerViewUtil(getWindow().getDecorView(), this, this, this, new MainListAdapter(7, this));//new MainListAdapter();  这个是适配器
...
//                mMainListAdapter.addData(getPageData(5));
    ...
    void sendliebiao() {//提现记录
        HttpUtils.GetUserFinList(new SubscriberRes<ZhunZhangBean>(rvu.getRecyclerView()) {
            @Override
    ...
    void cshlx() {
        HttpUtils.GUserFinList(new SubscriberRes<ArrayList<tj>>() {
            @Override
            public void onSuccess(ArrayList<tj> userBean) {
                tjss = userBean;
            }
    ...
    void tc(){
        ActionSheetDialog asd =  new ActionSheetDialog(QiTaJiLuActivity.this)
                .builder()
                .setCancelable(true)
                .setCanceledOnTouchOutside(true);
        for( final tj sj:tjss){
            asd.addSheetItem(""+sj.getName(),
